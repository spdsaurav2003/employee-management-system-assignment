...
exports.bulkImportEmployees = exports.deleteEmployee = exports.updateEmployee = exports.createEmployee = exports.getEmployeeById = exports.getEmployeesDropdown = exports.getEmployees = void 0;
...
const getEmployeesDropdown = async (req, res) => {
    try {
        const employees = await Employee_1.default.find({ isDeleted: false, status: 'Active' })
...
exports.getEmployeesDropdown = getEmployeesDropdown;
// GET /api/employees/:id - Retrieve individual employee details
const getEmployeeById = async (req, res) => {
    try {
        const employee = await Employee_1.default.findOne({ _id: req.params.id, isDeleted: false })
            .populate('reportingManager', 'name email employeeId');
        if (!employee) {
...
const createEmployee = async (req, res) => {
    ...
    const { name, email, password, phone, department, designation, salary, joiningDate, status, role, reportingManager, profileImage } = req.body;
    ...
    try {
        ...
        const emailExists = await Employee_1.default.findOne({ email: email.toLowerCase() });
        if (emailExists) {
        ...
        return res.status(400).json({ message: 'Failed to create employee', error: error.message });
    }
};
...
const updateEmployee = async (req, res) => {
    ...
    try {
        ...
        if (req.body.reportingManager !== undefined) {
            ...
            if (newManager) {
                const isCircular = await (0, circularCheck_1.isCircularReporting)(targetId, newManager);
                if (isCircular) {
        ...
        return res.status(400).json({ message: 'Failed to update employee', error: error.message });
    }
};
...
const deleteEmployee = async (req, res) => {
    ...
    try {
        ...
        await Employee_1.default.updateMany({ reportingManager: targetId }, { reportingManager: null });
...
const bulkImportEmployees = async (req, res) => {
    ...
    for (let i = 0; i < employees.length; i++) {
        ...
        try {
            ...
            // Check unique email
            const existing = await Employee_1.default.findOne({ email: empData.email.toLowerCase() });
            if (existing) {
                results.failCount++;
                results.errors.push({
                    row: rowNum,
            ...
            if (empData.managerEmail) {
                const mgr = await Employee_1.default.findOne({ email: empData.managerEmail.toLowerCase(), isDeleted: false });
                if (mgr) {
                    newEmp.reportingManager = mgr._id;
                }
            }
            await newEmp.save();
